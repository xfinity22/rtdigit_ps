...
function uploadCandidatePhoto(_url, types, form){
		...
		var uploadObj  = $("#fileuploader").uploadFile({
			...
			onSuccess:function(files,data,xhr,pd){
				...
				if(_data.respcode=="1"){		
					...
					$("#cropbox").removeClass("nodisplay");
					$("#cropbox").attr("src", _data.img);
					$(".candidate_avatar").val(_data.img);
			...
			onSelect:function(files){
				 ...
				 $.confirm({
                            title: 'Notification',
                            content: 'You are about to upload the photo. Click Confirm to Proceed',
                            icon: '',
                            animation: 'scale',
                            closeAnimation: 'scale',
                            opacity: 0.5,
			...
			function updateCoords(c) {
				...
				function responsiveCoords(c, imgSelector) {
					...
					cropThisImage(responsiveX, responsiveY, responsiveW, responsiveH);
					
			};
			
			
			function cropThisImage(x, y, w, h){
				

				var url = _webroot + 'members/cropimage';
					...
					success:function(resp){
						 if ($.isEmptyObject(resp)){
							...
							$("#photowrapper").fadeOut( function(){
								...
								$("#cropped_img, #p1, #p2").removeClass("nodisplay");
								//$("#cropped_img").attr('src', re.img);
								const img = document.getElementById('cropped_img');
								img.setAttribute('src', re.img + '?' + (new Date()).getTime());
								img.style.display = 'block';
								
								$(".candidate_newavatar").val(re.img);
							});
							
							//$("#final_avatar").val(re.img);
						 }
