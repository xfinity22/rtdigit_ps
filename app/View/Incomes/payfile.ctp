...
if(!empty($incomes)){
	foreach ($incomes as $income):
		...
		//if(!empty($employee['Employee']['payrollaccountnumber'])){
			...
			$objPHPExcel->setActiveSheetIndex(0)
				...
				->setCellValue('B' . $col, '\'' . $employee['Employee']['payrollaccountnumber'])
				->setCellValue('D' . $col, strtoupper($employee['Employee']['lastname'] . ', ' . $employee['Employee']['firstname'] . ' ' .$employee['Employee']['middlename']));
			...
			$objPHPExcel->getActiveSheet()->getStyle('F' . $col)->applyFromArray($BStyle);
			$objPHPExcel->getActiveSheet()->getStyle('H' . $col)->applyFromArray($BStyle);
	...
	$objPHPExcel->getActiveSheet()->getStyle('A1' . ':D' . $end)->getFont()->setSize(10)->setName('Calibri');
...
header('Content-Disposition: attachment;filename="ReceivingCopy.xlsx"');
