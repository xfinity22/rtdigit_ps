...
echo '<table width="98%" border="0"  cellpadding="0" cellspacing="0">';
	...
	if(!empty($deductions)){
		...
		echo '<table class="bordered" width="100%">';
			echo '<thead>';
			echo '<tr >';
				echo '<th colspan = "12" style="text-alig">LOANS</th>';
			echo '</tr>';
			echo '<tr>';
				echo '<th>Type</th>';
...
echo '<table width="98%" border="0">';
	...
	if($income['Income']['grossincome'] > 0 ){
		...
		$totalnetincome = $total - ($otherdeductions + $gov + $totaltax) + $nontaxable;
	
		if($income['Employee']['dailyrate'] == 481 || empty($income['Employee']['withholdingtax_id'])){
			$totaltax = 0;
		}
